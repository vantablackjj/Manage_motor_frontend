...
const PrintMaintenanceOrder = ({ order, warehouse, items }) => {
  ...
  const finalTotal = totalLabor + totalParts;
    ...
    >
        ...
        <tbody>
          <tr>
            <td style={{ ...cellStyle, width: "45%" }}>
              Tên khách hàng: <strong>{order.customer_name}</strong>
            </td>
            <td style={{ ...cellStyle, width: "25%" }}>
              Địa chỉ hiện tại: {order.customer_address}
            </td>
          ...
          <tr>
            <td style={cellStyle} rowSpan={2}>
              ...
              <span style={{ marginLeft: 6 }}>
                {["Ga", "Số", "Côn tay", "Xe Điện", "Phân khối lớn"].map(
                  (t) => {
                    const isElectric = !!order.battery_id;
                    const checked = isElectric
                      ? t === "Xe Điện"
                      : order.vehicle_type === t;
                    return (
                      <label
                        key={t}
            ...
            <td style={cellStyle}>
              Số điện thoại: <strong>{order.customer_phone}</strong>
            </td>
            <td style={cellStyle} colSpan={2}>
              Thời gian nhận xe: <strong>{fmtDate(order.received_at)}</strong>
            </td>
          </tr>
          <tr>
            ...
            <td style={cellStyle} colSpan={2}>
              ...
              <strong>{fmtDate(order.returned_at)}</strong>
            </td>
          </tr>
          <tr>
            ...
            <td style={cellStyle} colSpan={2}>
              ID Pin (Xe điện): {order.battery_id}
            </td>
          </tr>
          <tr>
            <td style={cellStyle}>
              Biển số: <strong>{order.license_plate}</strong> | Số Km:{" "}
              <strong>{fmt(order.km_reading)}</strong>
            </td>
            <td style={cellStyle} colSpan={3}>
              Rửa xe:
              <label style={{ marginLeft: 10, marginRight: 8 }}>
                <input
                  type="checkbox"
                  checked={order.wash_option === "Trước sửa chữa"}
                  readOnly
                />{" "}
                Trước sửa chữa
              </label>
              <label style={{ marginRight: 8 }}>
                <input
                  type="checkbox"
                  checked={order.wash_option === "Sau sửa chữa"}
                  readOnly
                />{" "}
                Sau sửa chữa
              </label>
              <label>
                <input
                  type="checkbox"
                  checked={order.wash_option === "Không cần rửa"}
                  readOnly
                />{" "}
                Không cần rửa xe
              </label>
          ...
          <tr>
            <td style={{ ...cellStyle, height: "30px" }}>
              Yêu cầu của khách hàng: {order.notes}
            </td>
            <td style={cellStyle} colSpan={3}>
              Tư vấn sửa chữa: ...
            </td>
      ...
      <table style={tableStyle}>
        <thead>
          <tr>
            <th
              style={{ ...headerStyle, fontSize: "8px" }}
              rowSpan={2}
              width="20"
            >
              STT
            </th>
            <th style={{ ...headerStyle, fontSize: "8px" }} rowSpan={2}>
              Tên phụ tùng, công việc
            </th>
            <th
              style={{ ...headerStyle, fontSize: "8px" }}
              rowSpan={2}
            ...
            <th
              style={{ ...headerStyle, fontSize: "8px", background: "#ffcccc" }}
              width="25"
            >
              Cần thay thế
            </th>
            <th
              style={{ ...headerStyle, fontSize: "8px", background: "#ffffcc" }}
              width="25"
            >
              Chú ý
            </th>
            <th style={{ ...headerStyle, fontSize: "8px" }} width="40">
              Báo giá
            </th>
            <th style={{ ...headerStyle, fontSize: "8px" }} colSpan={2}>
              Thay thế
            </th>
            <th style={{ ...headerStyle, fontSize: "8px" }} width="25">
              SL
            </th>
            <th style={{ ...headerStyle, fontSize: "8px" }} colSpan={3}>
            ...
            return (
              <tr key={idx} style={{ height: "20px" }}>
                ...
                <td style={{ ...cellStyle, textAlign: "right" }}>
                  {item?.type === "PART" ? fmt(item.total_price) : ""}
                </td>
                <td style={{ ...cellStyle, textAlign: "right" }}>
                  {item?.type === "SERVICE" ? fmt(item.total_price) : ""}
                </td>
                <td style={{ ...cellStyle, textAlign: "right" }}>
                  {item ? fmt(item.total_price) : ""}
                </td>
              </tr>
            );
          })}
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
              ...
              <div style={{ border: "1px solid #000", fontSize: "8px" }}>
                ...
                {[
                  ...
                  <div
                    ...
                    style={{
                      display: "flex",
                      borderBottom: i === 10 ? "none" : "1px solid #000",
                    }}
                  >
                    <div
                      style={{
                    ...
                    <div
                      style={{ width: "40px", borderRight: "1px solid #000" }}
                    ></div>
            ...
            <div style={{ width: "180px" }}>
              ...
              >
                Thời gian kiểm tra xe lần tới &lt;Amber Job&gt;
              </div>
              <table
                style={{
                  width: "100%",
              ...
              >
                ...
                <tbody>
                  {(() => {
                    ...
                    try {
                      jobs =
                        typeof order.amber_jobs === "string"
                          ? JSON.parse(order.amber_jobs)
                          : order.amber_jobs || [];
                    } catch (e) {
                    ...
                    return [0, 1, 2, 3, 4].map((idx) => {
                      ...
                      return (
                        <tr key={idx} style={{ height: "15px" }}>
                          ...
                          <td style={{ ...cellStyle, textAlign: "center" }}>
                            {job?.km ? Number(job.km).toLocaleString() : ""}
                          </td>
                        </tr>
                      );
                    });
      ...
      >
        Bằng việc ký vào Phiếu sửa chữa này, khách hàng đồng ý với Điều khoản và
        Chính sách Quyền Riêng Tư... (Vui lòng xem chi tiết tại
        www.honda.com/policy)
      </div>
    </div>
  );
};
