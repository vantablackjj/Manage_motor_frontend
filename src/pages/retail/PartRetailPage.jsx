...
const PartRetailPage = () => {
  ...
  const canManageMoney =
    isPowerUser ||
    user.can_manage_money === true ||
    user.can_manage_money === 1;
  const canDelete =
    isPowerUser || user.can_delete === true || user.can_delete === 1;
  ...
  const fetchInitialData = async (warehouseId = selectedWarehouseId) => {
    ...
    try {
      ...
      if (isAdmin) {
        ...
        setWarehouses(
          allWh.filter((w) => allowedWarehouseIds.includes(String(w.id))),
        );
      }
        ...
        parts.map((p) => {
          ...
          if (p.PartInventories && p.PartInventories.length > 0) {
            if (selectedWarehouseId) {
              const whStock = p.PartInventories.find(
                (inv) => inv.warehouse_id === selectedWarehouseId,
              );
              const qty = Number(whStock?.quantity || 0);
              stockText = qty.toLocaleString();
              stockColor =
                qty > 5 ? "#10b981" : qty > 0 ? "#f59e0b" : "#ef4444";
            } else {
              const totalQty = p.PartInventories.reduce(
          ...
          return {
            ...
            label: (
              ...
              >
                ...
                >
                  <div
                    style={{ fontWeight: 800, color: stockColor, fontSize: 13 }}
                  >
                    Tồn: {stockText}
                  </div>
                  {selectedWarehouseId &&
                    p.PartInventories?.find(
                      (inv) => inv.warehouse_id === selectedWarehouseId,
                    )?.location && (
                      <div
                        style={{
  ...
  const totalStockInWarehouse = allParts.reduce((sum, p) => {
    ...
    const inv = p.PartInventories.find(
      (i) => i.warehouse_id === selectedWarehouseId,
    );
  ...
  const addItem = (part) => {
    ...
    if (items.find((i) => i.part_id === part.id)) {
      return message.warning(
        "Phụ tùng này đã có trong hóa đơn. Hãy sửa số lượng.",
      );
    }
    const key = Date.now();
    setItems([
  ...
  const handlePrint = (saleData) => {
    ...
    const fullWh = warehouses.find(
      (w) => w.id === (saleData.warehouse_id || selectedWarehouseId),
    );
    if (fullWh) {
      wh = fullWh;
    }
  ...
  const onAddPayment = async (values) => {
    try {
      await api.put(`/part-sale/${selectedSaleForDetail.id}/payment`, {
  ...
  const handleExport = () => {
    ...
    exportToExcel(
      exportData,
      `BanLePhuTung_${dayjs().format("YYYYMMDD_HHmm")}`,
    );
  };
  ...
  const historyColumns = [
    ...
    },
    {
      ...
      render: (v, r) => v || r.customer_phone || "Khách lẻ",
    },
      ...
      render: (_, r) => (
        <Space>
          ...
          {canDelete && (
            <Popconfirm
              title="Hủy hóa đơn?"
              description="Kho sẽ được hoàn lại. Tiếp tục?"
  ...
  return (
    <div className="page-container">
            ...
            children: (
              <Card className="glass-card" styles={{ body: { padding: 0 } }}>
                ...
                >
                  ...
                  />
                  <Space>
                    <Badge count={filteredHistory.length} overflowCount={999} color="var(--primary-color)">
                      <Text type="secondary" style={{ marginRight: 8 }}>Tổng kết quả:</Text>
                    </Badge>
                    <Button
                      icon={<RotateCcw size={14} />}
      ...
      <Modal
        ...
        footer={[
          ...
          canManageMoney && (
            <Button
              ...
              onClick={() => {
                ...
                handleOpenPaymentModal(selectedSaleForDetail);
              }}
            >
              Thu tiền
            </Button>
          ...
          <Button
            ...
            onClick={() => handlePrint(selectedSaleForDetail)}
          >
            In hóa đơn
          </Button>,
        ]}
          ...
          <>
            <Row gutter={16} style={{ marginBottom: 16 }}>
              <Col span={8}>
                ...
                <Text strong>
                  {dayjs(selectedSaleForDetail.sale_date).format("DD/MM/YYYY")}
                </Text>
              </Col>
              <Col span={8}>
                <Text type="secondary">Khách:</Text>{" "}
                <Text strong>
                  {selectedSaleForDetail.customer_name || "Khách lẻ"}
                </Text>
              </Col>
              <Col span={8}>
                <Text type="secondary">SĐT:</Text>{" "}
                <Text>{selectedSaleForDetail.customer_phone || "—"}</Text>
              </Col>
            </Row>
            <Table
            ...
            <Row justify="end" gutter={32}>
              <Col>
                ...
                <Text strong style={{ fontSize: 15 }}>
                  {Number(selectedSaleForDetail.total_amount).toLocaleString()}{" "}
                  đ
                </Text>
              </Col>
              <Col>
                ...
                <Text strong type="success">
                  {Number(selectedSaleForDetail.paid_amount).toLocaleString()} đ
                </Text>
              </Col>
              <Col>
                {Number(selectedSaleForDetail.total_amount) -
                  Number(selectedSaleForDetail.paid_amount) >
                0 ? (
                  <>
      ...
      <Modal
        title={
          <Space>
            <Banknote size={18} /> Thu tiền hóa đơn #
            {selectedSaleForDetail?.id?.slice(-8)?.toUpperCase() || "..."}
          </Space>
        }
