...
const ExpensePage = () => {
  ...
  const fetchData = async (f = filters) => {
    ...
    try {
      ...
      const filteredWH = isPowerUser
        ? wRes.data
        : wRes.data.filter((w) =>
            allowedWarehouseIds.includes(w.id.toString()),
          );
  ...
  const handleExport = (type) => {
    ...
    const exportData = dataToExport.map((e) => ({
      [type === "expense" ? "Ngày chi" : "Ngày thu"]: dayjs(
        e.expense_date || e.income_date,
      ).format("DD/MM/YYYY"),
      "Xe liên quan": e.related_vehicle
        ? e.related_vehicle.engine_no
        : "Toàn hệ thống/Chung",
    ...
    exportToExcel(
      exportData,
      `DanhSach_${type === "expense" ? "ChiPhi" : "ThuNhap"}_${dayjs().format("YYYYMMDD_HHmm")}`,
    );
  };
  ...
  const onFinishIncome = async (values) => {
    try {
      const payload = {
        ...values,
        income_date: values.income_date.toISOString(),
      };
  ...
  const commonColumns = (type) => [
        ...
        r.related_vehicle ? (
          <Space direction="vertical" size={0}>
            <Text strong style={{ fontSize: 12 }}>
              {r.related_vehicle.engine_no}
            </Text>
            <Text type="secondary" style={{ fontSize: 10 }}>
              SM: {r.related_vehicle.chassis_no}
            </Text>
          </Space>
        ) : (
    ...
    },
    {
      ...
      width: "150px",
      render: (w) =>
        w?.warehouse_name || <Tag color="orange">Toàn hệ thống</Tag>,
    },
    {
      title: "Số tiền (VNĐ)",
      dataIndex: "amount",
      ...
      render: (val) => (
        <Text
          strong
          style={{ color: type === "expense" ? "#ef4444" : "#10b981" }}
        >
    ...
    { title: "Nội dung", dataIndex: "content", key: "content" },
    {
      ...
      align: "center",
      render: (_, r) =>
        isAdmin || user.can_delete ? (
          <Popconfirm
            title={`Xóa khoản ${type === "expense" ? "chi" : "thu"} này?`}
            onConfirm={() =>
              type === "expense"
                ? handleDeleteExpense(r.id)
                : handleDeleteIncome(r.id)
            }
          >
  ...
  const renderForm = (type) => (
    <Card
      ...
      style={{
        borderLeft: `4px solid ${type === "expense" ? "#ef4444" : "#10b981"}`,
      }}
      title={
        <Space>
          {type === "expense" ? (
          ...
          <span style={{ fontSize: 14 }}>
            {type === "expense" ? "GHI PHIẾU CHI" : "GHI PHIẾU THU"}
          </span>
        </Space>
      }
    >
      ...
      >
        <Row gutter={12}>
          ...
          <Col xs={24} sm={12}>
            <Form.Item
              label="Ngày"
              name={type === "expense" ? "expense_date" : "income_date"}
              rules={[{ required: true }]}
            >
    ...
    <div className="page-container" style={{ maxWidth: 1400 }}>
      ...
      >
        <Col xs={24} xl={6}>
          ...
          <Text type="secondary">
            Theo dõi dòng tiền ngoài hệ thống bán lẻ.
          </Text>
        </Col>
        ...
        <Col xs={24} xl={18}>
          <Row gutter={[16, 16]} justify="end">
            <Col xs={24} sm={8}>
              <Card
                className="glass-card"
                style={{ background: "#f0fdf4", margin: 0 }}
              >
                <Statistic
            ...
            <Col xs={24} sm={8}>
              <Card
                className="glass-card"
                style={{ background: "#fef2f2", margin: 0 }}
              >
                <Statistic
            ...
            <Col xs={24} sm={8}>
              <Card
                className="glass-card"
                style={{ background: "#eff6ff", margin: 0 }}
              >
                <Statistic
                  title={
                    <span style={{ color: "#1e40af" }}>Dòng tiền ròng</span>
                  }
                  ...
                  valueStyle={{
                    color:
                      totalIncome - totalExpense >= 0 ? "#1d4ed8" : "#b91c1c",
                    fontSize: 18,
                  }}
