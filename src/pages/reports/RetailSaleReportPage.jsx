...
const RetailSaleReportPage = () => {
  ...
  const fetchWarehouses = async () => {
    try {
      ...
      const filtered = isPowerUser
        ? res.data
        : res.data.filter((w) => allowedWarehouseIds.includes(w.id.toString()));
  ...
  const fetchData = async (currentFilters = filters) => {
    ...
    try {
      ...
      const sortedSales = [...res.data.sales].sort(
        (a, b) => dayjs(b.sale_date).unix() - dayjs(a.sale_date).unix(),
      );
  ...
  const handleOpenPaymentModal = async (sale) => {
    try {
      ...
      const loan =
        sale.is_disbursed === true || sale.is_disbursed === 1
          ? Number(sale.loan_amount || 0)
          : 0;
  ...
  const toggleGuaranteeBook = async (id, currentStatus) => {
    try {
      ...
      message.success(
        !currentStatus ? "Đã xác nhận cấp sổ!" : "Đã hủy xác nhận cấp sổ!",
      );
      fetchData();
    } catch (e) {
  ...
  const handleDisbursementChange = async (saleId, status) => {
    if (status) {
      ...
      setDisburseSale(sale);
      setDisburseDate(dayjs());
      setDisburseModalVisible(true);
    } else {
      try {
        setLoading(true);
        await api.put(`/retail-sales/${saleId}/disbursement`, {
          is_disbursed: false,
        });
  ...
  const columns = [
      ...
      render: (d, r) => {
        ...
        return (
          <Space>
            ...
            {hasError && (
              <Tooltip
                title={`Lỗi logic: Ngày nhập (${dayjs(purchaseDate).format("DD/MM/YYYY")}) sau ngày bán!`}
              >
      ...
      : null,
    {
      ...
      render: (_, r) => (
        <Space size={[0, 4]} wrap>
          {r.gifts &&
            Array.isArray(r.gifts) &&
            r.gifts.map((g, idx) => {
              const name = typeof g === 'string' ? g : g.name;
              const qty = typeof g === 'string' ? 1 : g.quantity;
              return (
                <Tag color="cyan" key={idx} style={{ fontSize: 10 }}>
                  {name} {qty > 1 ? `(x${qty})` : ""}
                </Tag>
              );
            })}
    ...
    isPowerUser
      ? {
          ...
          render: (_, r) => {
            ...
            const loan = isDisbursed ? Number(r.loan_amount || 0) : 0;
    ...
    },
    {
      ...
      fixed: "right",
      render: (_, r) => {
        if (r.payment_method !== "Trả góp")
          return <Text type="secondary">-</Text>;

        return (
          <Space direction="vertical" size={0}>
            {r.bank_name && (
              <Text strong style={{ fontSize: 11, color: "#3b82f6" }}>
                {r.bank_name}
              </Text>
            )}
            <Checkbox
  ...
  return (
    <div style={{ padding: "0 5px" }}>
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <Statistic
              title={
                <Space>
                  <CheckCircle2 size={14} /> Xe làm đăng ký
                </Space>
              }
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <Statistic
              title={
                <Space>
                  <ShoppingCart size={14} /> Xe bán hồ sơ
                </Space>
              }
          ...
          <Col xs={24} sm={12} md={6}>
            ...
            >
              <Statistic
                title={
                  <Space>
                    <DollarSign size={14} /> Thực thu hiện tại
                  </Space>
                }
        ...
        <Card size="small" className="glass-card" style={{ marginBottom: 20 }}>
          ...
          >
            <Space>
              ...
              <Text strong>SỐ QUÀ KM ĐÃ XUẤT</Text>
            </Space>
          </div>
          ...
          <Col xs={24} md={5}>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <Checkbox
                ...
                onChange={(e) =>
                  handleFilterChange("hasDebt", e.target.checked)
                }
            ...
            >
              ...
              <div>
                ...
                <DatePicker
                  ...
                  onChange={(d) =>
                    handleFilterChange("dates", [d, filters.dates[1]])
                  }
                  format="DD/MM/YYYY"
                />
              </div>
              <div>
                ...
                <DatePicker
                  ...
                  onChange={(d) =>
                    handleFilterChange("dates", [filters.dates[0], d])
                  }
                  format="DD/MM/YYYY"
                />
              </div>
      ...
      <Card className="glass-card table-card" bodyStyle={{ padding: 0 }}>
        ...
        >
          <Text strong style={{ color: "#0f172a" }}>
            Danh sách chi tiết xe bán lẻ từ ngày{" "}
            {filters.dates[0]?.format("DD/MM/YYYY") || "..."} đến ngày{" "}
      ...
      <Modal
        title={
          <Space>
            <History size={18} /> Lịch sử & Thu tiền:{" "}
            ...
            <Row gutter={16}>
              <Col span={12}>
                ...
                >
                  ...
                  >
                    {Number(selectedSale.total_price).toLocaleString()} đ
                  </div>
                </Card>
              </Col>
              <Col span={12}>
                ...
                >
                  ...
                  >
                    {(() => {
                      const isDisbursed =
                        selectedSale.is_disbursed === true ||
                        selectedSale.is_disbursed === 1;
                      const price = Number(selectedSale.total_price || 0);
                      const paid = Number(selectedSale.paid_amount || 0);
                      const loan = isDisbursed
                        ? Number(selectedSale.loan_amount || 0)
                        : 0;
                      return (price - paid - loan).toLocaleString();
                    })()}{" "}
                    đ
                  </div>
                </Card>
